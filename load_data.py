...
def batch_data_process(batch_data):
    ...
    for i in range(batch_size):
        ...
        if i < (batch_size - 1):
            batch_start_index.append(
                batch_start_index[-1] + len(batch_data[i]))
...
def get_train_data(config, mode, pretrain_dataset, shuffle=True):
    ...
    data_x_path = get_data_x_path(
        config.feature_path, split_set, mode, pretrain_dataset)
    ...
    batch_start_list = [
        i * batch_size for i in range(int(num_data / batch_size))]
    ...
    for bstart in batch_start_list:
        ...
        tmp_batch_gt_label, start_index = batch_data_process(
            tmp_batch_gt_label)
...
def get_val_data(config, mode, pretrain_dataset, shuffle=True):
    batch_size = config.batch_size
